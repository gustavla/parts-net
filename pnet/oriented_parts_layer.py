...
#def _extract_batch2(X, settings, part_shape, extract_func):
def _extract_batch2(im, settings, num_parts, num_orientations, part_shape,
                   parts, extract_func):
    with ag.Timer('transpose'):
        gpu_im = im.transpose((0, 3, 1, 2)).astype(np.float64)

    print('IM size', ag.memsize(gpu_im))

    res = extract_func(gpu_im)

    pad_shape = (im.shape[0], im.shape[1]+1, im.shape[2]+1, im.shape[3])
    #imp = ag.util.pad_repeat_border_corner(im, pad_shape)
...
def _extract_batch(X, settings, num_parts, num_orientations, part_shape,
    ...
    #if (standardize == 0 and n_coded == 1 and
         #min_percentile == 0.0 and rotspread == 0):
        # Run the new Theano code
...
class OrientedPartsLayer(Layer):
    ...
    def train_from_samples(self, raw_patches, raw_originals):
        ...
        if self._settings['circular']:
            ...
            #self._parts_vis = mask0 * self._parts + ~mask0 * np.nan
                    ...
                    if th <= tot:
                        ...
                        for ori in range(ORI * POL):
                            ...
                            try:
                                ...
                                #print('Skipping', selection, X.shape)
                                br = True
                        ...
                        # Visually check boxes
                        if 0:
                            mm = np.zeros(X_shape)
                            for ori, selection in enumerate(sels):
                                mm[selection[1:]] = (ori + 1) / len(sels)

                            from vzlog import default as vz
                            grid = ag.plot.ImageGrid(mm, border_color=(1, 0, 0))
                            grid.save(vz.impath(), scale=10)

                        if 0:
                            # Make sure it works
                            try:
                                assert_equal(1, sels[0][1].start, sels[1][2].start)
                                assert_equal(2, sels[0][1].start, X_shape[0]-sels[2][1].stop)
                                assert_equal(3, sels[0][1].start, X_shape[1]-sels[3][2].stop)
                            except:
                                import pdb; pdb.set_trace()
    ...
    def _preprocess(self):
        ...
        s_parts = theano.shared(gpu_parts, name='parts')
        s_logits = theano.shared(gpu_logits, name='logits')
        s_rest = theano.shared(gpu_rest, name='rest')

        #s_conv = (conv.conv2d(s_input, T.log(s_parts / (1 - s_parts))) +
                   #T.log(1 - s_parts).sum(1).sum(1).sum(1).dimshuffle('x', 0, 'x', 'x'))
